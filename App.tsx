...
const App: React.FC = () => {
  ...
  const handleStartExperiment = (info: ParticipantInfo) => {
    setShuffledTrials(TRIALS);
    setParticipant(info);
    setAppState(AppState.INSTRUCTION); // Step 1: Gather info -> Go to instructions
  };

  const handleProceedToExperiment = () => {
    setAppState(AppState.CHAT); // Step 2: Instructions -> Start first trial
  };
  ...
  if (appState === AppState.INSTRUCTION) {
    return (
      <div className="min-h-screen flex items-center justify-center p-6 bg-gradient-to-br from-indigo-50 to-blue-100">
        <InstructionScreen onContinue={handleProceedToExperiment} />
      </div>
    );
  }
